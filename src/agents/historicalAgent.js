...
Provide comprehensive, historically-informed analysis based only on the API responses you receive.`;
  ...
  async analyzeRegulationChanges(regulationYears) {
    try {
      const regulationData = await Promise.all(
        regulationYears.map(year => this.seasonTools.getSeasonByRegulation(year))
      );
  ...
  async trackEvolution(entity, entityId, startYear, endYear) {
    try {
      ...
      switch (entity) {
        case 'driver':
          evolutionData = await this.trackDriverEvolution(entityId, startYear, endYear);
          break;
        case 'constructor':
          evolutionData = await this.trackConstructorEvolution(entityId, startYear, endYear);
          break;
        case 'circuit':
          evolutionData = await this.trackCircuitEvolution(entityId, startYear, endYear);
          break;
        default:
  ...
  async processQuery(query, context = {}) {
    try {
      ...
      if (historicalInfo.era) {
        ...
        const eraData = await this.analyzeEra(historicalInfo.era.start, historicalInfo.era.end);
  ...
  extractHistoricalFromQuery(query) {
    ...
    const decades = decadeMatches ? decadeMatches.map(d => parseInt(d.replace('s', ''))) : [];
    ...
    return {
      ...
      era,
      decades,
      isHistorical: queryLower.includes('historical') || queryLower.includes('all time') || queryLower.includes('evolution')
    };
  }
  ...
  getEraCharacteristics(startYear, endYear) {
    ...
    if (startYear >= 2014) {
      characteristics.push('V6 turbo hybrid power units', 'Energy recovery systems', 'Advanced aerodynamics');
    } else if (startYear >= 2006) {
      characteristics.push('Standardized V8 engines', 'KERS introduction', 'Slick tires return');
    } else if (startYear >= 1982) {
      characteristics.push('Turbocharged engines', 'Extreme power levels', 'Ground effect aerodynamics');
    }
    ...
    eraData.forEach(season => {
      if (season.driverStandings?.DriverStandings) {
        ...
        if (champion) {
          ...
          if (driverId) {
            driverChampionships[driverId] = (driverChampionships[driverId] || 0) + 1;
          }
        }
      }
    });
  ...
  identifyDominantConstructors(eraData) {
    ...
    eraData.forEach(season => {
      if (season.constructorStandings?.ConstructorStandings) {
        ...
        if (champion) {
          ...
          if (constructorId) {
            constructorChampionships[constructorId] = (constructorChampionships[constructorId] || 0) + 1;
          }
        }
      }
    });
    ...
    return Object.entries(constructorChampionships)
      .sort(([,a], [,b]) => b - a)
      .slice(0, 5)
      .map(([constructorId, championships]) => ({ constructorId, championships }));
  }
  ...
  assessCompetitiveBalance(eraData) {
    ...
    eraData.forEach(season => {
      season.races?.forEach(race => {
        if (race.Results?.[0]) {
          ...
          uniqueConstructorWinners.add(race.Results[0].Constructor?.constructorId);
        }
      });
    });
    ...
    return {
      ...
      competitiveLevel: this.categorizeCompetitiveLevel(uniqueWinners.size, uniqueConstructorWinners.size)
    };
  }
  ...
  identifyMajorTechnicalChanges(startYear, endYear) {
    ...
    if (startYear <= 2014 && endYear >= 2014) changes.push('Introduction of hybrid power units');
    if (startYear <= 2009 && endYear >= 2009) changes.push('Return of slick tires');
    if (startYear <= 1994 && endYear >= 1994) changes.push('Introduction of electronic driver aids');
    if (startYear <= 1982 && endYear >= 1982) changes.push('Turbo engine era begins');
  ...
  generateRegulationAnalysis(regulationData) {
    return {
      ...
      effectiveness: 'Assessment of regulation effectiveness in achieving intended goals'
    };
  }
  ...
  async trackConstructorEvolution(constructorId, startYear, endYear) {
    return {
      performance: 'Constructor performance evolution over the specified period',
